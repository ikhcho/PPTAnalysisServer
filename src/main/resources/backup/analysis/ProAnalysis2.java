 ...
 * 
*/
public class ProAnalysis2 implements Analysis{
	...
	public ProAnalysis2(StockVO stockVO) {
		proDic = new ProDic2(stockVO.getName());
		this.list = stockVO.getQuote();
	}
	
	public ProAnalysis2(String comName) {
			...
			for(NewsMorpVO morp: morpList){
				...
				while(iter.hasNext()){
					...
					if(dictionary.containsKey(key)){
						...
						incScore += prodicVO.getInc();
						decScore += prodicVO.getDec();
						equScore += prodicVO.getEqu();
						cnt++;
					}else{
						 continue;
					}
	...
	public void predict(String predicDate){
		...
		/*System.out.print("Probability Analysis2 =>> ");
		...
		System.out.print(", 주가 : " + prediction);*/
		double max = Double.max(Double.max(incScore, decScore),equScore);
		if ((max == incScore && prediction.contains("p")) 
