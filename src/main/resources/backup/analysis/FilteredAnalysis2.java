...
public class FilteredAnalysis2 implements Analysis{
	...
	public FilteredAnalysis2(StockVO stockVO, TFIDF tfidf) {
		proDic = new ProDic2(stockVO.getName());
		this.list = stockVO.getQuote();
		this.tfidf = tfidf;
	}
	
	public FilteredAnalysis2(String comName, TFIDF tfidf) {
	...
	public void predict(String predicDate){
		...
		/*if (((incScore / cnt)<49.0852 && prediction.contains("pm")) || ((incScore / cnt)>49.0852 && prediction.contains("m"))){
			//System.out.println("예측성공");
			success++;
		} else{
			//System.out.println("예측실패");
